...
def load_rdw_data():
    ...
    # Kolommen met datum omzetten naar datetime-formaat
    df['datum_eerste_tenaamstelling_in_nederland'] = pd.to_datetime(df['datum_eerste_tenaamstelling_in_nederland'], format='%Y%m%d', errors='coerce')
    df['merk_model'] = df['merk'] + ' ' + df['type']
    ...
    df = df[df['brandstof_omschrijving'].isin(['Benzine','Elektriciteit','Diesel','Hybride'])]

    df = df.dropna(subset=['datum_eerste_tenaamstelling_in_nederland'])

    df['inschrijvingsmaand'] = df['datum_eerste_tenaamstelling_in_nederland'].dt.to_period('M').astype(str)
    #Hoe vaak komt elk merk voor
...
with tab3:
    ...
    # Maak de bar chart voor de top 10 merken zonder modellen
    # SLIDER: filter op de prijsklasse
    min_price, max_price = st.slider(
        'Selecteer een prijsklasse',
        min_value=int(df['catalogusprijs'].min()),
        max_value=int(df['catalogusprijs'].max()),
        value=(int(df['catalogusprijs'].min()), int(df['catalogusprijs'].max()))
    )

    # Filter de dataset op basis van de geselecteerde prijsklasse
    price_df = df[(df['catalogusprijs'] >= min_price) & (df['catalogusprijs'] <= max_price)]

    # Bereken het aantal voorkomens per merk binnen de prijsklasse
    merk_counts = price_df['merk'].value_counts().head(10)

    # Chart voor de top 10 merken 
    fig = px.bar(merk_counts,
                x=merk_counts.index,
                y=merk_counts.values,
                labels={'x': 'Merk', 'y': 'Aantal'},
                title='Top 10 meest voorkomende merken',
                color_discrete_sequence=['skyblue'])
    ...
    # Filter de data voor catalogusprijzen tussen 15k en 100k
    filtered_prices = df[df['catalogusprijs'].between(15000, 100000)]['catalogusprijs'].dropna()

    # Maak een histogram met hover informatie en randkleur
    ...
    # Titel en labels
    fig.update_layout(
        title='Gemiddelde aanschafprijs elektrische auto',
        xaxis_title='Catalogusprijs in Euro',
...
# Groeperen per brandstofcategorie en per maand en cumulatief optellen
    cumulative_df = df.groupby(['inschrijvingsmaand', 'brandstof_omschrijving']).size().groupby(level=1).cumsum().reset_index(name='aantal_voertuigen')
    cumulative_df['inschrijvingsmaand'] = pd.to_datetime(cumulative_df['inschrijvingsmaand'])

    # Add checkbox for prediction
    show_prediction = st.checkbox("Toon voorspelling tot 2026", value=False)

    if show_prediction:
        future_dates = pd.date_range(start=cumulative_df['inschrijvingsmaand'].max(), end='2026-12-31', freq='M')
        
        fig = go.Figure()

        for fuel_type in cumulative_df['brandstof_omschrijving'].unique():
            df_fuel = cumulative_df[cumulative_df['brandstof_omschrijving'] == fuel_type]
            
            # Add original data
            fig.add_trace(go.Scatter(
                x=df_fuel['inschrijvingsmaand'], 
                y=df_fuel['aantal_voertuigen'],
                mode='lines',
                name=fuel_type
            ))
            
            # Calculate and add prediction
            x = (df_fuel['inschrijvingsmaand'] - df_fuel['inschrijvingsmaand'].min()).dt.days
            y = df_fuel['aantal_voertuigen']
            
            slope, intercept, _, _, _ = stats.linregress(x, y)
            
            # Calculate future_x correctly
            future_x = (future_dates - df_fuel['inschrijvingsmaand'].min()).days
            ...
            fig.add_trace(go.Scatter(
                ...
                mode='lines',
                line=dict(dash='dash'),
                name=f'{fuel_type} Voorspelling'
            ))

        fig.update_layout(
            title='Cumulatief aantal voertuigen per maand per brandstofcategorie (met voorspelling)',
            xaxis_title='Maand van inschrijving',
